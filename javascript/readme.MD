...
```javascript
    ...
    connectToServer = () => {
        ...
        if (this.getServerType() === 'diffusion') {
...
1. If we used the Mosquitto MQTT server, with a pure MQTT Producer / Client, we get 0% compression given, in this case Diffusion is out of the equation.
